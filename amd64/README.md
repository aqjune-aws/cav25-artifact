# CAV25 Artifact (amd64 architecture): s2n-bignum

> This README file is for the **amd64** architecture. For arm64, please download the archive called `s2n-bignum-arm64.zip` from Zenodo.
...
We tested this specific artifact on amd64 (x86-64) Linux machine.
...
1. Load and enter the provided docker image (<1 min):
    ...
    ```bash
    docker load < s2n-bignum-amd64.tar
    docker run -it s2n-bignum-amd64:latest /bin/bash
    ```
...
This artifact has been compiled with `docker build -t s2n-bignum-amd64:latest .` (~10 min) on a Linux amd64 machine and saved with `docker save s2n-bignum-amd64:latest > s2n-bignum-amd64.tar` (<1 min). The Dockerfile form amd64 is available inside the zipped archive, and the image is ~2.6GB
